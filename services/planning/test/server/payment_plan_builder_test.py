...
class Cases:
    ...
    def case_min_fees_2_months_biweekly(self):
        return (
            self.user_id,
            MetaData(preferred_plan_type=PlanType.PLAN_TYPE_MIN_FEES,
            ...
            PaymentPlan(
                ...
                plan_type=PlanType.PLAN_TYPE_MIN_FEES,
                end_date=shift_now_by_payment_frequency_multiple_times(PaymentFrequency.PAYMENT_FREQUENCY_BIWEEKLY, 4),
                active=True,
                status=PaymentStatus.PAYMENT_STATUS_CURRENT,
                payment_action=[
                    PaymentAction(
                        account_id=self.accName2Id['Chase'],
                        amount=125,
                        transaction_date=shift_now_by_payment_frequency_multiple_times(PaymentFrequency.PAYMENT_FREQUENCY_BIWEEKLY, 1),
                        status=PaymentActionStatus.PAYMENT_ACTION_STATUS_PENDING),
                    PaymentAction(
                        account_id=self.accName2Id['Chase'],
                        amount=125,
                        transaction_date=shift_now_by_payment_frequency_multiple_times(PaymentFrequency.PAYMENT_FREQUENCY_BIWEEKLY, 2),
                        status=PaymentActionStatus.PAYMENT_ACTION_STATUS_PENDING),
                    PaymentAction(
                        account_id=self.accName2Id['Chase'],
                        amount=50,
                        transaction_date=shift_now_by_payment_frequency_multiple_times(PaymentFrequency.PAYMENT_FREQUENCY_BIWEEKLY, 3),
                        status=PaymentActionStatus.PAYMENT_ACTION_STATUS_PENDING),
                    PaymentAction(
                        account_id=self.accName2Id['Amex'],
                        amount=75,
                    ...
                    PaymentAction(
                        account_id=self.accName2Id['Amex'],
                        amount=125,
                        transaction_date=shift_now_by_payment_frequency_multiple_times(PaymentFrequency.PAYMENT_FREQUENCY_BIWEEKLY, 4),
                        status=PaymentActionStatus.PAYMENT_ACTION_STATUS_PENDING)
                ]
            )
        )
    ...
    def case_credit_score_5_months_monthly(self):
        return (
            self.user_id,
            MetaData(preferred_plan_type=PlanType.PLAN_TYPE_OPTIM_CREDIT_SCORE,
                     preferred_timeline_in_months=5.0,
                     preferred_payment_freq=PaymentFrequency.PAYMENT_FREQUENCY_MONTHLY),
            ['', '', ''],
            [self.accName2Id['Chase'], self.accName2Id['Amex'], self.accName2Id['Barclay']],
            [180, 170, 170],
            PaymentPlan(
                user_id=self.user_id,
                payment_task_id=['', '', ''],
                ...
                plan_type=PlanType.PLAN_TYPE_OPTIM_CREDIT_SCORE,
                end_date=shift_now_by_payment_frequency_multiple_times(PaymentFrequency.PAYMENT_FREQUENCY_MONTHLY, 5),
                active=True,
                status=PaymentStatus.PAYMENT_STATUS_CURRENT,
                payment_action=[
                    PaymentAction(
                        account_id=self.accName2Id['Amex'],
                        amount=104,
                        transaction_date=shift_now_by_payment_frequency_multiple_times(PaymentFrequency.PAYMENT_FREQUENCY_MONTHLY, 1),
                        status=PaymentActionStatus.PAYMENT_ACTION_STATUS_PENDING),
                    PaymentAction(
                        account_id=self.accName2Id['Amex'],
                        amount=66,
                        transaction_date=shift_now_by_payment_frequency_multiple_times(PaymentFrequency.PAYMENT_FREQUENCY_MONTHLY, 2),
                        status=PaymentActionStatus.PAYMENT_ACTION_STATUS_PENDING),
                    PaymentAction(
                        account_id=self.accName2Id['Chase'],
                        amount=38,
                    ...
                    PaymentAction(
                        account_id=self.accName2Id['Chase'],
                        amount=104,
                        transaction_date=shift_now_by_payment_frequency_multiple_times(PaymentFrequency.PAYMENT_FREQUENCY_MONTHLY, 3),
                        status=PaymentActionStatus.PAYMENT_ACTION_STATUS_PENDING),
                    PaymentAction(
                        account_id=self.accName2Id['Chase'],
                        amount=38,
                        transaction_date=shift_now_by_payment_frequency_multiple_times(PaymentFrequency.PAYMENT_FREQUENCY_MONTHLY, 4),
                        status=PaymentActionStatus.PAYMENT_ACTION_STATUS_PENDING),
                    PaymentAction(
                    ...
                    PaymentAction(
                        account_id=self.accName2Id['Barclay'],
                        amount=104,
                        transaction_date=shift_now_by_payment_frequency_multiple_times(PaymentFrequency.PAYMENT_FREQUENCY_MONTHLY, 5),
                        status=PaymentActionStatus.PAYMENT_ACTION_STATUS_PENDING)
                ]
            )
        )
    ...
    def case_credit_score_3_months_monthly_pay_off_whole_acc(self):
        return (
            ...
            PaymentPlan(
                ...
                plan_type=PlanType.PLAN_TYPE_OPTIM_CREDIT_SCORE,
                end_date=shift_now_by_payment_frequency_multiple_times(PaymentFrequency.PAYMENT_FREQUENCY_MONTHLY, 3),
                active=True,
                status=PaymentStatus.PAYMENT_STATUS_CURRENT,
                payment_action=[
                    PaymentAction(
    ...
    def case_no_meta_data_amount_lt_250(self):
        return (MetaData(preferred_plan_type=PlanType.PLAN_TYPE_UNKNOWN, preferred_timeline_in_months=0.0,
    ...
    def case_min_fees_no_other_meta_data_amount_gt_250(self):
        return (MetaData(preferred_plan_type=PlanType.PLAN_TYPE_MIN_FEES, preferred_timeline_in_months=0.0,
                         preferred_payment_freq=PaymentFrequency.PAYMENT_FREQUENCY_UNKNOWN),
                500,
                [
                    MetaData(preferred_plan_type=PlanType.PLAN_TYPE_MIN_FEES,
    ...
    def case_credit_score_biweekly_payments_no_timeline_amount_gt_250(self):
        return (MetaData(preferred_plan_type=PlanType.PLAN_TYPE_OPTIM_CREDIT_SCORE, preferred_timeline_in_months=0.0,
                         preferred_payment_freq=PaymentFrequency.PAYMENT_FREQUENCY_BIWEEKLY),
                500,
                [
                    MetaData(preferred_plan_type=PlanType.PLAN_TYPE_OPTIM_CREDIT_SCORE,
    ...
    def case_quarterly_6_month_no_type(self):
        return (MetaData(preferred_plan_type=PlanType.PLAN_TYPE_UNKNOWN, preferred_timeline_in_months=6.0,
...
@parametrize_with_cases("paymentTasks, metaData, paymentPlans", cases=Cases, has_tag='end_to_end')
def test_create_payment_plan_end_to_end(paymentTasks, metaData, paymentPlans, gen_payment_plan_builder):
...
@parametrize_with_cases("userId, metaData, paymentTaskIds, accountIds, amounts, paymentPlan", cases=Cases,
                        has_tag='create_single_payment_plan')
def test_create_single_payment_plan(userId: str, metaData: MetaData, paymentTaskIds: List[str], accountIds: List[str],
                                    amounts: List[float], paymentPlan: PaymentPlan, gen_payment_plan_builder):
    paymentPlanCreated = gen_payment_plan_builder._createPaymentPlanMetaDataChosen(userId=userId, metaData=metaData,
...
@parametrize_with_cases("metaData, totalAmount, metaDataOptions", cases=Cases, has_tag='meta_data_options')
def test_create_meta_data_options(metaData: MetaData, totalAmount: float, metaDataOptions: List[MetaData], gen_payment_plan_builder):
    metaDataOptionsCreated = gen_payment_plan_builder._createMetaDataOptions(metaData=metaData, totalAmount=totalAmount)

    assert len(metaDataOptions) == len(metaDataOptionsCreated), f"Test asks for {len(metaDataOptions)} but " \
                                                                f"{len(metaDataOptionsCreated)} were created"
    for i, (metaData, metaDataCreated) in enumerate(zip(metaDataOptions, metaDataOptionsCreated)):
        assert metaData.preferred_plan_type == metaDataCreated.preferred_plan_type, f"MetaData {i} differs"
        assert metaData.preferred_timeline_in_months == metaDataCreated.preferred_timeline_in_months, f"MetaData {i} differs"
        assert metaData.preferred_payment_freq == metaDataCreated.preferred_payment_freq, f"MetaData {i} differs"
