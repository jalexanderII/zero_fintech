...
from gen.Python.common.common_pb2 import PAYMENT_ACTION_STATUS_PENDING, PAYMENT_STATUS_CURRENT, \
    PAYMENT_STATUS_IN_DEFAULT, PAYMENT_STATUS_COMPLETED, PAYMENT_ACTION_STATUS_COMPLETED, \
    PAYMENT_ACTION_STATUS_IN_DEFAULT, PAYMENT_STATUS_CANCELLED
...
from services.planning.app.models.pydantic import PercentCoveredByPlansResponseSchema, \
    StatisticalOverviewCategoricalDataResponseSchema, StatisticalOverviewContinuousDataResponseSchema, \
    OverviewAllPreferencesResponseSchema
from services.planning.database.db import initiate_async_mongo_client
from services.planning.database.models.common import PaymentPlan as PaymentPlanDB

router = APIRouter()

PLANNING_COLLECTION: AsyncIOMotorCollection = initiate_async_mongo_client()
...
async def get_overview_amount_per_payment() -> StatisticalOverviewContinuousDataResponseSchema:
    amount_per_payment = []
...
async def get_overview_payment_plan_amounts() -> StatisticalOverviewContinuousDataResponseSchema:
    amounts = []
    async for _pp in PLANNING_COLLECTION.find({}):
        pp = PaymentPlanDB().from_dict(_pp)
...
async def get_overview_percent_covered_by_plans() -> PercentCoveredByPlansResponseSchema:
    ...
    for user_id in user_plan_amounts.keys():
        ...
        total_amount += amount
        percent_covered_users[user_id] = user_plan_amounts[user_id] / amount if amount > 0 else 1
    percent_covered_users = np.array(percent_covered_users.values())
...
async def get_overview_current_defaulted_plans() -> StatisticalOverviewCategoricalDataResponseSchema:
    category_to_count = defaultdict(int)
    async for _pp in PLANNING_COLLECTION.find({}):
        pp = PaymentPlanDB().from_dict(_pp)
        if pp.status in [PAYMENT_STATUS_CURRENT, PAYMENT_STATUS_IN_DEFAULT]:
            category_to_count[pp.status] += 1
    return StatisticalOverviewCategoricalDataResponseSchema(category_to_count=category_to_count)


@router.get('/overview_amounts_completed_pending_defaulted')
async def get_overview_amounts_completed_pending_in_default() -> StatisticalOverviewCategoricalDataResponseSchema:
    ...
    async for _pp in PLANNING_COLLECTION.find({}):
        ...
        if pp.status == PAYMENT_STATUS_COMPLETED:
            ...
            # defaulted?
            for pa in pp.payment_action:
                if pa.status in [PAYMENT_ACTION_STATUS_PENDING, PAYMENT_ACTION_STATUS_IN_DEFAULT]:
...
async def get_overview_timeline_preferences() -> StatisticalOverviewContinuousDataResponseSchema:
    timeline = []
    async for _pp in PLANNING_COLLECTION.find({}):
        pp = PaymentPlanDB().from_dict(_pp)
...
async def get_overview_plan_type_preferences() -> StatisticalOverviewCategoricalDataResponseSchema:
    plan_type_counts = defaultdict(int)
    async for _pp in PLANNING_COLLECTION.find({}):
        pp = PaymentPlanDB().from_dict(_pp)
        plan_type_counts[pp.plan_type] += 1
    return StatisticalOverviewCategoricalDataResponseSchema(category_to_count=plan_type_counts)


@router.get('/overview_payment_frequency_preferences')
async def get_overview_payment_frequency_preferences() -> StatisticalOverviewCategoricalDataResponseSchema:
    payment_freq_counts = defaultdict(int)
    async for _pp in PLANNING_COLLECTION.find({}):
        pp = PaymentPlanDB().from_dict(_pp)
        payment_freq_counts[pp.payment_freq] += 1
    return StatisticalOverviewCategoricalDataResponseSchema(category_to_count=payment_freq_counts)


@router.get('/overview_all_preferences')
async def get_overview_all_preferences() -> OverviewAllPreferencesResponseSchema:
    timeline, plan_type_counts, payment_freq_counts = [], defaultdict(int), defaultdict(int)
    async for _pp in PLANNING_COLLECTION.find({}):
        pp = PaymentPlanDB().from_dict(_pp)
        timeline.append(pp.timeline)
        plan_type_counts[pp.plan_type] += 1
